...
public class MethodInfo {
  ...
  public MethodInfo(DataInput dataInput, JClass jClass) {
    ...
    int nameIdx = dataInput.readUnsignedShort();
    name = ((UTF8Constant) jClass.constantPool().constant(nameIdx)).value();

    int descriptorIdx = dataInput.readUnsignedShort();
    descriptor = ((UTF8Constant) jClass.constantPool().constant(descriptorIdx)).value();
    ...
    for (int i = 0; i < attributesCount; i++) {
      ...
      code = attributes[i] instanceof Code ? (Code) attributes[i] : null;
    }
  }
