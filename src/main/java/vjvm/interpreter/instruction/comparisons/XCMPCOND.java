package vjvm.interpreter.instruction.comparisons;

import lombok.AccessLevel;
import lombok.AllArgsConstructor;
import lombok.var;
import vjvm.interpreter.instruction.Instruction;
import vjvm.runtime.JThread;
import vjvm.runtime.OperandStack;
import vjvm.runtime.ProgramCounter;
import vjvm.runtime.classdata.MethodInfo;

import java.util.function.BiConsumer;
import java.util.function.BinaryOperator;
import java.util.function.Function;

/**
 * @author zym
 * @date 2022/06/05 09:32
 **/
@AllArgsConstructor(access = AccessLevel.PRIVATE)
public class XCMPCOND<T> extends Instruction {
  private final BinaryOperator<T> opFunc;
  private final Function<OperandStack, T> popFunc;
  private final BiConsumer<OperandStack, Integer> pushFunc;
  private String name;

  public static final XCMPCOND<Float> FCMPL(ProgramCounter pc, MethodInfo method) {
    return new XCMPCOND<>((a, b) -> (float) a.compareTo(b), OperandStack::popFloat, OperandStack::pushInt, "fcmpl");
  }

  public static final XCMPCOND<Float> FCMPG(ProgramCounter pc, MethodInfo method) {
    return new XCMPCOND<>((a, b) -> (float) (a > b ? 1 : (a.compareTo(b))), OperandStack::popFloat, OperandStack::pushInt, "fcmpg");
  }

  public static final XCMPCOND<Double> DCMPL(ProgramCounter pc, MethodInfo method) {
    return new XCMPCOND<>((a, b) -> (double) a.compareTo(b), OperandStack::popDouble, OperandStack::pushInt, "dcmpl");
  }

  public static final XCMPCOND<Double> DCMPG(ProgramCounter pc, MethodInfo method) {
    return new XCMPCOND<>((a, b) -> (double) (a > b ? 1 : (a.compareTo(b))), OperandStack::popDouble, OperandStack::pushInt, "dcmpg");
  }

  @Override
  public void run(JThread thread) {
    var stack = thread.top().stack();
    T num2 = popFunc.apply(stack);
    T num1 = popFunc.apply(stack);
    T res = opFunc.apply(num1, num2);
    int value = (res instanceof Float) ? ((Float) res).intValue() : ((Double) res).intValue();
    pushFunc.accept(stack, value);
  }


  @Override
  public String toString() {
    return name;
  }
}
